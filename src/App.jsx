        ...
        if (result.status === 1) {
          ...
          if (result.data !== undefined) {
            ...
            if (result?.data?.length == 1) {
              api.getSystemLanguage(result?.data?.[0]?.id, 1).then((res) => res.json()).then((langRes) => {
                dispatch(setLanguage({ data: langRes.data }))
              })
            }
          }
          else {
            ...
            <Routes key={"routes"}>
              {user.user ?
                <>
                  ...
                  <Route key="reset-password" exact path="/profile/reset-password" element={<ProfileDashboard showPassword={true} />} />
